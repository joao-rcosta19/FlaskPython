...
from model import db, Estudante
#from aula15.model import db, Estudante
import json
...
def add():
    ...
    db.session.commit()
    return app.response_class(response=json.dumps({'status': 'sucesso', 'data': estudante.to_dict()}), status=200, content_type='application/json')
